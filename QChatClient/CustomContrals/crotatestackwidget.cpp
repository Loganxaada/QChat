...
CRotateStackWidget::CRotateStackWidget(QWidget *parent)
...
CRotateStackWidget::~CRotateStackWidget()
{

}

void CRotateStackWidget::paintEvent(QPaintEvent *event)
{
    if (m_bAnimating)
    {
        if (m_rotateVal > 90)
...
void CRotateStackWidget::rotate(int index)
{
...
float CRotateStackWidget::rotateVal()
{
    return m_rotateVal;
}

void CRotateStackWidget::setRotateVal(float fl)
{
     m_rotateVal = fl;
}

void CRotateStackWidget::nextPage()
{
    rotate((currentIndex() + 1) >= count() ? 0 : (currentIndex() + 1));
}

void CRotateStackWidget::valChanged(QVariant)
{
    repaint();
}

void CRotateStackWidget::animationDone()
{
    m_rotateVal = 0;
    m_bAnimating = false;
